...
# Start.io AdManager (Dummy Implementation)
...
class AdManager:
    ...
    def show_rewarded_ad(self):
        # Start.io does not have classic rewarded ads like AdMob,
        # but you can simulate rewards after interstitial
        shown = self.show_interstitial_ad()
        if shown:
            print("Reward user manually after ad if needed")
        return shown
...
# AI Progression System with Endless Levels
    ...
    def generate_current_level(self):
        ...
        if player_data:
            ...
            if len(player_data) >= 9:
                main_level = player_data[8]  # endless_level
                sub_level = player_data[7]   # sub_level
            else:
                main_level = player_data[3] if len(player_data) > 3 else 1  # level
                sub_level = 1
...
class MenuScreen(Screen):
    def __init__(self, game_data, **kwargs):
        ...
        # Title
        title = Label(text='FREE FIRE MOBILE', font_size='24sp', size_hint_y=0.2)
        layout.add_widget(title)
        
        # Player info
        player_info = self.get_player_info_text()
        info_label = Label(text=player_info, font_size='14sp', size_hint_y=0.3)
    ...
    def get_player_info_text(self):
        ...
        if player_data:
            ...
            if len(player_data) >= 9:
                ...
                gold = player_data[3]
                xp = player_data[5]
                main_level = player_data[8]
                sub_level = player_data[7]
            else:
                ...
                main_level = player_data[3] if len(player_data) > 3 else 1
                sub_level = 1
    ...
    def _show_rewarded_ad_and_reward(self, instance):
        ...
        if app.ad_manager.show_rewarded_ad():
            ...
            # Assuming info_label has an id for easy access
            # self.parent.get_screen('menu').ids.info_label.text = self.get_player_info_text()
            # For now, just print to console as we don't have IDs set up in the Kivy layout
            print("Player rewarded with 50 diamonds!")
        else:
            self.ad_popup.dismiss()
...
class GameScreen(Screen):
    ...
    def on_enter(self):
        ...
        # Start game loop
        Clock.schedule_interval(self.update_game, 1/30.0)  # 30 FPS for low-end devices
    ...
    def add_ui_overlay(self):
        ...
        # Placeholder for banner ad at the bottom
        self.banner_ad_placeholder = BoxLayout(size_hint_y=None, height=50, pos_hint={'bottom':1})
        self.add_widget(self.banner_ad_placeholder)
    ...
    def end_game(self, victory):
        ...
        if player_data and victory and progression["advance"]:
            # Handle both old and new database schemas
            if len(player_data) >= 9:
                current_sub_level = player_data[7]
                current_main_level = player_data[8]
            else:
                current_sub_level = 1
                current_main_level = player_data[3] if len(player_data) > 3 else 1
...
class ShopScreen(Screen):
    def __init__(self, game_data, **kwargs):
        ...
        if player_data:
            # Handle both old and new database schemas
            if len(player_data) >= 9:
                level = player_data[8]  # endless_level
            else:
                level = player_data[3] if len(player_data) > 3 else 1
